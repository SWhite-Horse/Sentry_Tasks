...
void Task_JudgeReceive(void *parameters)
{
  while (1)
  {
    ...
    JudgeReceive();
//		printf("Power: %f\r\n",PowerHeatData_data.chassisPower);
  }
}
...
void JudgeConnection_Init(UART_HandleTypeDef *huart)
{
  ...
  HAL_DMA_Start_IT(huart->hdmarx, (uint32_t)&huart->Instance->DR, (uint32_t)&Judge_Receive_Buffer, 130);
...
void Judge_IDLECallback(UART_HandleTypeDef *huart)
{
  BaseType_t xHigherPriorityTaskToWaken = pdFALSE;
	...
	if(__HAL_UART_GET_IT_SOURCE(huart, UART_IT_IDLE))
	{
        ...
        //发送消息通知
        vTaskNotifyGiveFromISR(TaskHandle_JudgeReceive,&xHigherPriorityTaskToWaken);
  ...
  switch(CmdID)
  {
    ...
    case ROBOTHURT:
    {
      ...
      break;
		...
		case BULLETREMANING:
		{
		  memcpy(&ext_bullet_remaining,(Judge_Receive_Buffer+JUDGE_DATA_OFFSET+SOF),BULLETREMANING_DATE_SIZE);
			break;
