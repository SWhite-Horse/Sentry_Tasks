...
void imu_temp_ctrl(void)
{
	...
	if (imu.temp >= imu.Tar_Temp)
	{
		...
		if (!(xEventGroupGetBits(EG_Wait) & (0x01 << imu_ob_temp_pos)))
		{
			xEventGroupSetBits(EG_Wait,0x01 << imu_ob_temp_pos);
		}
	}
	else if (imu.temp < imu.Tar_Temp)
	{
		...
		else __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_2, HEAT_MAX);
	}
}
